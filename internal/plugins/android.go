...
func findMatchingPackage(ctx context.Context, serial string, pattern string) string {
	...
	for _, line := range lines {
		...
		// Performance optimization: using path.Match instead of dynamically compiling regexes
		// avoids the expensive overhead of regex compilation inside a function.
