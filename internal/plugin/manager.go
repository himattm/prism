...
func (m *Manager) addFromDirectURL(rawURL string) error {
	...
	resp, err := safeHTTPGet(parsedURL.String())
	if err != nil {
		return fmt.Errorf("failed to fetch plugin: %w", err)
	}
	defer resp.Body.Close()
...
// safeHTTPGet performs an HTTP GET request with SSRF protections
	...
	dialer := &net.Dialer{
		...
		Control: func(network, address string, c syscall.RawConn) error {
			...
			if ip.IsLoopback() || ip.IsPrivate() || ip.IsUnspecified() || ip.IsLinkLocalUnicast() {
				return fmt.Errorf("SSRF prevention: blocked access to IP %s", ip.String())
			}
			return nil
		},
	}
