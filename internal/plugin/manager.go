...
func (m *Manager) addScriptPlugin(owner, repo, pluginName string) error {
	...
	// Parse metadata directly from memory to prevent temporary file exposure
...
func secureGet(urlStr string) (*http.Response, error) {
	dialer := &net.Dialer{
		...
		Control: func(network, address string, c syscall.RawConn) error {
			...
			if ip != nil && ip.Equal(net.ParseIP("169.254.169.254")) {
				return fmt.Errorf("blocked request to cloud metadata IP")
			}
			return nil
		},
	}
