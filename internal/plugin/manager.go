...
var (
	metadataKeyValRe  = regexp.MustCompile(`^#\s*@(\w+[-\w]*)\s+(.+)$`)
	metadataVersionRe = regexp.MustCompile(`(?m)^#\s*@version\s+(.+)$`)
)
...
func ParseMetadata(path string) (Metadata, error) {
	...
	for scanner.Scan() && lineCount < 20 {
		...
		matches := metadataKeyValRe.FindStringSubmatch(line)
		if len(matches) == 3 {
...
func (m *Manager) checkScriptVersion(p Plugin, client *http.Client) (string, error) {
	...
	matches := metadataVersionRe.FindSubmatch(content)
	if len(matches) < 2 {
