...
// safeHTTPClient returns an HTTP client configured to prevent SSRF by blocking
// connections to loopback, private, and link-local addresses.
	...
	dialer := &net.Dialer{
		...
		Control: func(network, address string, c syscall.RawConn) error {
			...
			if ip.IsLoopback() || ip.IsPrivate() || ip.IsUnspecified() || ip.IsLinkLocalUnicast() {
				return fmt.Errorf("access to private or loopback IP denied: %s", ip.String())
			}
