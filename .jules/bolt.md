...
## 2024-05-22 - Avoid strings.Split in hot paths
**Learning:** Using `strings.Split` to parse strings separated by a known delimiter causes unnecessary slice allocations, degrading performance, particularly inside parsing loops (like version string comparisons).
**Action:** Replace `strings.Split` with an allocation-free loop using `strings.Cut` to incrementally process the delimited string.
