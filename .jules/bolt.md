...
## 2025-05-10 - Reuse http.Client to Enable Connection Pooling
**Learning:** In Go, recreating an `http.Client` for every request is a performance anti-pattern that prevents TCP connection reuse (keep-alive) and forces new TLS handshakes, drastically increasing latency. `http.Client` is explicitly designed to be thread-safe for concurrent goroutine use.
**Action:** Always lift `http.Client` instances to package-level variables or struct fields for reuse across multiple API calls to enable connection pooling.
