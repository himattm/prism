...
## 2024-05-20 - Cache Regex Compilation to Reduce CPU Overhead
**Learning:** Calling `regexp.MustCompile` inside frequently executed functions (like file parsing during discovery) forces the regex engine to parse and compile the pattern repeatedly, causing unnecessary CPU overhead and memory allocations.
**Action:** Always declare `regexp.MustCompile` as package-level global variables so they are compiled exactly once at application startup.
