...
## 2024-05-25 - Avoid strings.Split in parsing loops
**Learning:** Using `strings.Split` creates a slice and allocates memory for every element, which causes noticeable overhead in tight parsing loops like version string comparisons.
**Action:** Use a `for` loop with `strings.Cut` to incrementally consume and parse the string without allocating any intermediate slices. This is significantly faster and generates less garbage.
