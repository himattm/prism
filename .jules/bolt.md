...
## 2024-05-20 - Global Regular Expression Compilation
**Learning:** Compiling regular expressions with `regexp.MustCompile` inside functions executed frequently (like during plugin discovery) wastes CPU cycles and memory by repeatedly compiling the same expression.
**Action:** Always declare standard regular expressions as package-level global variables so they are compiled exactly once at application startup.
