...
## 2024-05-24 - Avoid runtime regex compilation
**Learning:** Calling `regexp.MustCompile` inside frequently executed functions (like plugin discovery loops) causes unnecessary CPU overhead and memory allocations because the regex is recompiled on every function call.
**Action:** Always declare regular expressions as package-level global variables using `var ... = regexp.MustCompile(...)` so they are compiled exactly once at application startup.
